...
using System.Xml;
using System;
public class UI_btnCtrl : MonoBehaviour
{
    ...
    void OnClick()
    {
        if (btnType == 1)
        {
            ...
            if(GlobalData.g_global.myCardList.Count == 0){
                ...
                mainSceneUI = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster/input_panel");
                buttons = mainSceneUI.GetComponentsInChildren<BoxCollider>();
                for (int i = 0; i < buttons.Length; ++i)
                {
                    buttons[i].enabled = false;
            ...
            if (GlobalData.g_global.myCardList[GlobalData.g_global.selectedCardNum].cardused != 0)
            {
                ...
                mainSceneUI = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster/infopopup_panel");
                buttons = mainSceneUI.GetComponentsInChildren<BoxCollider>();
                for (int i = 0; i < buttons.Length; ++i)
                {
                    buttons[i].enabled = false;
                }
                mainSceneUI = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster/errorpopup_panel");
                buttons = mainSceneUI.GetComponentsInChildren<BoxCollider>();
                for (int i = 0; i < buttons.Length; ++i)
                {
...
//                        monster_ui.m_infopopup.gameObject.SetActive(false);
        ...
        else if (btnType == 5)
        {
            ...
            GameObject card = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster/upgrade_panel/upgrade_target");
        ...
        else if (btnType == 6)             
        {
            ...
            GameObject.Find("mainSceneUI/Camera/Anchor/mainBase/layer_Item/gold_label").GetComponent<UILabel>().text = GlobalData.g_global.myInfo.coinAmount.ToString();
        ...
        else if (btnType == 9)       // 141016
        {
            if (GlobalData.g_global.myInfo.coinAmount < int.Parse(monster_ui.m_mix_cost.GetComponent<UILabel>().text))
            {
                ...
                mainSceneUI = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster/mix_panel");
                buttons = mainSceneUI.GetComponentsInChildren<BoxCollider>();
                for (int i = 0; i < buttons.Length; ++i)
                {
                    buttons[i].enabled = false;
        ...
        else
        {
            ...
            else
            {
                ...
                mainSceneUI = GameObject.Find("mainSceneUI/Camera/Anchor/popup_monster");
