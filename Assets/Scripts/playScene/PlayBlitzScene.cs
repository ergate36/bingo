...
public class PlayBlitzScene : MonoBehaviour
{
    ...
    private PlayBlitzScene_UI playScene_ui;
    ...
    void Start()
    {
        ...
        playScene_ui = gameObject.GetComponent<PlayBlitzScene_UI>();
...
//        StartCoroutine("addBall", nb_GlobalData.g_global.bingoball[count]);
    ...
    void Update()
    {
        ...
        */
        ...
        if (nb_GlobalData.g_global.socketState == (int)nb_SocketClass.STATE.BlitzEndGameAlarm_End)
        {
            //게임오버
            runGameOver();
            ...
            nb_GlobalData.g_global.socketState == (int)nb_SocketClass.STATE.BlitzCompleteBingoResponse_End)
        {
            ...
            {
                playScene_ui.m_rankBoard.Find("rank_1/username").GetComponent<UILabel>().text =
                    nb_GlobalData.g_global.BingoLastFinishUserName;
            }
            else if (nb_GlobalData.g_global.BingoTotalFinishCount == 2)
            {
                playScene_ui.m_rankBoard.Find("rank_2/username").GetComponent<UILabel>().text =
                    nb_GlobalData.g_global.BingoLastFinishUserName;
            }
            else if (nb_GlobalData.g_global.BingoTotalFinishCount == 3)
            {
                playScene_ui.m_rankBoard.Find("rank_3/username").GetComponent<UILabel>().text =
                    nb_GlobalData.g_global.BingoLastFinishUserName;
            }
            else if (nb_GlobalData.g_global.BingoTotalFinishCount >= 4)
            {
                playScene_ui.m_rankBoard.Find("rank_x/username").GetComponent<UILabel>().text =
                    nb_GlobalData.g_global.BingoLastFinishUserName;

                playScene_ui.m_rankBoard.Find("rank_x/rank_text").GetComponent<UILabel>().text =
    ...
    public void processDaub(int sheetIndex, int index)
    {
        ...
        if (bDaubed == false)
        {
            ...
            daubObjects[sheetIndex, index].GetComponent<UISprite>().spriteName = "daub_x";
                ...
                //
            {
                ...
                //
                daubObjects[sheetIndex, index].GetComponent<UISprite>().spriteName = "daub_o";
    ...
    private void initBingoSheet()
    {
        ...
        for (int sheetIndex = 0; sheetIndex < 4; ++sheetIndex)
        {
            for (int cellIndex = 0; cellIndex < 25; ++cellIndex)
            {
                ...
                daubObjects[sheetIndex, cellIndex] = Instantiate(Resources.Load("game/blitz_daub")) as GameObject;
        ...
        UIGrid grid = playScene_ui.m_rankBoard.Find("playerList/Grid").GetComponent<UIGrid>();
        UIScrollView sv = playScene_ui.m_rankBoard.Find("playerList").GetComponent<UIScrollView>();

        for (int i = 0; i < userCount; ++i)
        {
            m_playerList[i] = Instantiate(Resources.Load("game/playerForm")) as GameObject;
            ...
            m_playerList[i].transform.Find("username").GetComponent<UILabel>().text =
        ...
        playScene_ui.m_rankBoard.Find("rank_x/rank_text").GetComponent<UILabel>().text = "4th";
    }
    ...
    public IEnumerator addBall(int number)
    {
        ...
        else
        {
            ...
            if (number != 0)
            {
                playScene_ui.m_daubBoardNumber[number - 1].GetComponent<UILabel>().color = Color.red;
            }
    ...
    private void setActiveNumberButtons(bool active)
    {
        for (int s = 0; s < 4; ++s)
        {
            for (int i = 0; i < 25; ++i)
            {
                playScene_ui.m_cells[s, i].GetComponent<BoxCollider>().enabled = active;
            }
        }
    }
